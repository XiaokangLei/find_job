...
Page({
  ...
  onLoad: async function (options) {
    ...
    if (blogId !== undefined) {
      ...
      console.log('post1:', that.data.post, new Date().getTime())
      // that.editorCtx.setContents({
      //   html: result.data.content == "" ? result.data.info : result.data.content,
      //   success: (res) => {
      //     console.log(res)
      //   },
      //   fail: (res) => {
  ...
  changePostDetail: function (e) {
    ...
    console.log('post2:', that.data.post)
    console.info(e)
    switch (e.target.id) {
      case 'postTitle': {
      ...
      case 'postData1': {
        post.data_1 = e.detail.value
        break
      }
      case 'postData2': {
        post.data_2 = e.detail.value
        break
      }
      case 'postData3': {
        post.defaultImageUrl = e.detail.value
        break
      }
      case 'postMettingaddr': {
        post.work = e.detail.value
        break
      }
      case 'postInfo': {
        post.info = e.detail.value
        break
      }
    }
  ...
  savePost: async function (e) {
    ...
    if (post._id === undefined) {
      ...
      console.log('post.info:',post.info)
      newPost = {
        // digest: post.digest, //摘要
        // timestamp: new Date().getTime(),
        title: post.title,
        ...
        data_1:post.data_1,
        data_2:post.data_2,
        defaultImageUrl:post.defaultImageUrl,
        work:post.work,
        createTime: util.formatTime(new Date()),
        info: post.info
      }
