...
const APPLY_TEMPLATE_ID = 'mAgYBfTZFWh3x1ARZQ_i3P_eIiGNixZScBlpINgdBhw'
  ...
  try {
    ...
    await db.collection('mini_member').doc(event.id).update({
      data: {
        ...
        level: level,
        modifyTime: new Date(Date.now() + (8 * 60 * 60 * 1000)).getTime()
      }
    });
    ...
    try {
      const result = await cloud.openapi.subscribeMessage.send({
        ...
        data: {
          ...
          date3: {
            value: new Date(Date.now() + (8 * 60 * 60 * 1000)).toFormat("YYYY-MM-DD HH24:MI")
          },
          thing7: {
